...
public class CustomPluginAdapter extends PluginAdapter {
	 ...
	 * 是否需要继承base实体
	 */
	public boolean hasBaseEntity = true;
	...
	private void commonJavaDoc(JavaElement element) {
		element.addJavaDocLine("/**");
		element.addJavaDocLine(" * @AUTO ");
		element.addJavaDocLine(" * @DATE " + new SimpleDateFormat("yyyy年M月d日HH:mm:ss").format(new Date()));
		element.addJavaDocLine(" * @Author " + properties.getProperty("author"));
		element.addJavaDocLine(" * @Version " + properties.getProperty("version"));
		element.addJavaDocLine(" */");
	}

	/**
	 * @Interface 对应生成的接口<br>
	 * @TopLevelClass 对应生成的类<br>
	 * @IntrospectedTable 数据库表映射信息<br>
	 * @Description 生成DAO
	 */
	@Override
	...
	public boolean modelBaseRecordClassGenerated(TopLevelClass topLevelClass, IntrospectedTable introspectedTable) {
		if (hasBaseEntity) {
			FullyQualifiedJavaType fqjt = new FullyQualifiedJavaType("BaseEntity<"
			...
			for (IntrospectedColumn introspectedColumn : introspectedTable.getNonPrimaryKeyColumns()) {
				// 判断是否有时间类型的参数
				if (introspectedColumn.getJdbcTypeName().toLowerCase().contains("date")
						|| introspectedColumn.getJdbcTypeName().toLowerCase().contains("time")) {
	...
	public boolean sqlXMLGenerated(Document document, IntrospectedTable introspectedTable) {
		...
		for (IntrospectedColumn introspectedColumn : introspectedTable.getNonPrimaryKeyColumns()) {
			...
			if (!(introspectedColumn.getJdbcTypeName().toLowerCase().contains("date") || introspectedColumn
					.getJdbcTypeName().toLowerCase().contains("time"))) {// mybatis保存日期报错
				sb.append(" and ").append(introspectedColumn.getJavaProperty() + " != ''");
			}
	...
	public boolean methodXMLGenerated_one(Document document, IntrospectedTable introspectedTable) {
		...
		select.addElement(new TextElement("order by id desc "));
		select.addElement(new TextElement("<if test=\"offset != null and limit != null\">"));
		select.addElement(new TextElement("	limit ${offset}, ${limit}\n\t</if>"));
	...
	public boolean methodXMLGenerated_two(Document document, IntrospectedTable introspectedTable) {
		...
		selectCount.addAttribute(new Attribute("resultType", "int"));
		selectCount.addElement(new TextElement("select count(1) from "
				                                       + introspectedTable.getFullyQualifiedTableNameAtRuntime()));
		XmlElement where = new XmlElement("where");
		XmlElement include2 = new XmlElement("include");
		include2.addAttribute(new Attribute("refid", "Base_Where_List"));
		where.addElement(include2);
		selectCount.addElement(where);
	...
	public List<GeneratedJavaFile> contextGenerateAdditionalJavaFiles(IntrospectedTable introspectedTable) {
		...
		String serviceName = servicePackage + "." + modelName + "Service";
		List<GeneratedJavaFile> additionalJavaFiles = new ArrayList<GeneratedJavaFile>();
		GeneratedJavaFile service = generateServiceInterface(modelName, serviceName);
		additionalJavaFiles.add(service);
	 ...
	 *            服务名称
	 */
	private GeneratedJavaFile generateServiceInterface(String modelName, String serviceName) {
		TopLevelClass genService = new TopLevelClass(new FullyQualifiedJavaType(serviceName));
		// 描述类的作用域修饰符
		if (hasBaseEntity) {
			genService.addImportedType(new FullyQualifiedJavaType("com.fit.base.BaseCrudService"));
			genService.addImportedType(new FullyQualifiedJavaType(servicePackage.replace("service", "bean.")
					                                                      + modelName));
			genService.addImportedType(new FullyQualifiedJavaType(servicePackage.replace("service", "dao.") + modelName
					                                                      + "Dao"));
			FullyQualifiedJavaType fqjt = new FullyQualifiedJavaType("BaseCrudService<" + modelName + "Dao, "
					                                                         + modelName + ">");
			genService.setSuperClass(fqjt);
		}
		genService.setVisibility(JavaVisibility.PUBLIC);
		genService.addImportedType(new FullyQualifiedJavaType("org.springframework.stereotype.Service"));
		genService.addAnnotation("@Service");
		commonJavaDoc(genService);
		GeneratedJavaFile gjf = new GeneratedJavaFile(genService, properties.getProperty("targetProject"),
